...
def main() -> None:
    ...
    artifacts: dict[str, dict[str, Any]] = {
        ...
        "validation-summary.json": envelope(
            ...
            focused_stage22_tests=43,
            full_tests_passed=204,
            full_tests_skipped=1,
            total_coverage_percent=86.14,
            required_format_skips=0,
            ruff="PASS",
            strict_mypy="PASS",
            frozen_part1="PASS",
            protected_part1="PASS",
            frozen_part2_stage1="PASS",
            stage22_validator="PASS",
            status="SOURCE_FREEZE_VALIDATED",
        ),
    }
    for name, value in artifacts.items():
        (EVIDENCE / name).write_text(render(value), encoding="utf-8")
    changed = set(git("diff", "--name-only", f"{BASE_COMMIT}...{source_commit}").splitlines())
    changed.update(git("diff", "--name-only", BASE_COMMIT).splitlines())
